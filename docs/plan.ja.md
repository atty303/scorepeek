...
### Live Overlay canvas（ADR 0122 / 0125 / 0127 / 0128 / 0129 / 0130 / 0135 / 0137 / 0138 / 0140 / 0143 / 0144 / 0145）
...
editorはbackend全canvasを1 draftとして扱い、active output、current GAME SCREEN、canvas/widget選択、gesture、panel stateと一段のdocument UNDOを共有reducerで管理する。Wayland/OBS adapterはoutput catalog、surface/transportとinputだけを担当し、editor DOM/CSSも共有する。compact ContextBar、`Workspace → Output → Canvas → Widget` navigator、Inspector、panel幅内ActionBarを持ち、navigatorとInspectorは独立scrollする。canvasの右dragとwidgetの左drag、およびInspectorの数値fieldでgeometryを編集し、canvas/widgetの位置とsizeは4px gridへ揃える。ただしcanvasの右端または下端が非gridのassigned-output外周へ正確に一致するdimensionは許可する。新規canvasはactive outputのouter boundsを`0,0,width,height`で正確にfillする。新規widgetは既定sizeでcanvas中央へ即時作成する。zero canvas時もoutputとskinを選択でき、CREATE FIRST CANVASはcurrent GAME SCREENだけに表示されるcanvasを作る。output再割当とFIT TO OUTPUTは明示的なundo対象とし、navigationはdraftもundoも変更しない。canvas名、全6 GAME SCREENのvisibility、appearance、output、削除とwidget固有設定はInspectorに集約し、runtime refresh cadenceはeditor外とする。
