...
9. **M6**: screen、accepted play attempt、title/artist/chart contextのfield recognizer、full-catalog screen-local song
   ...
   12 operator-context、133 predicate-context pairを保持し、未判断pairは0である。ADR 0065はADR 0064で早すぎた2つの
   ...
   この最後の数値はstableをunknownで消す旧reducerの歴史的結果である。ADR 0067はproduction hold-and-replace reducerで
   100/200/300/500 ms dwellと100/200/300 ms unknown graceを再評価し、correct-song truthとmotion truthを合わせて
   200/200 msをprovisional runtime presentationに選択した。`held_unknown`と`changing`は旧IDをcurrent acceptedとして扱わず、
...
### Live Overlay canvas（ADR 0122 / 0125 / 0127 / 0128 / 0129 / 0130 / 0135 / 0137 / 0138 / 0140 / 0143 / 0144 / 0145 / 0146 / 0147 / 0148 / 0149）

配布binaryは`scorepeek`だけとし、`run --overlay-wayland`と`run --overlay-obs`で独立consumerを有効化する。
`--overlay-config PATH`はschema v8のTOMLを選び、未指定時は`$XDG_CONFIG_HOME/scorepeek/overlay.toml`を使う。schema v2は`z`を除去し、schema v3はcanvasの`enabled`を除去し、schema v4はwidget外形を内側content領域へ変換し、schema v5は旧skin名を正式IDへ変換してappearance値をpropertyへ保存する。schema v7はoverlay、backend、canvasのrevisionと`initial_placement`を除去し、canvasのoutputを必須にする。schema v8はcanvasへbackend内で一意の非空`name`を追加し、旧canvasをbackendごとの設定順で`Canvas 1`から命名する。旧`enabled = false`は`show_on = []`へ変換し、それ以外の`show_on`は保持する。schema v1は拒否する。`--overlay-wayland-edit`はWaylandを有効化して起動時に編集workspaceを開くが、空のWayland workspaceはこのoptionなしでもeditorを自動起動する。
...
editorはbackend全canvasを1 draftとして扱い、Dioxus editor instance内の`Signal<EditorSession>`だけを書き込み可能な正本とする。active output、current GAME SCREEN、canvas/widget選択、gesture、panel state、title/IME stateと一段のdocument UNDOは全inputを共有reducerへ渡して更新し、active output、selection、Inspector view、selected canvas、output別`StageProjection`は`use_memo`で導出する。memoは書き込み可能stateでも同期対象でもなく、同一VDOMのcomponentは必要なmemoだけを購読する。Wayland/OBS adapterはeditor actionやfieldを解釈せず、editor/stage message transport、output catalog、surface、normalized input、paintおよび永続化effectだけを担当し、editor DOM/CSSも共有する。backend固有処理はprotocol、surface lifecycle、別instanceへのtransport、rendererのDOM input/focus/paint差をbrowserの契約へ正規化する不可避なadapter処理だけに限定し、selection、picker、field、drag、output切替その他のeditor semanticsを代替実装するfallbackを持たない。OBSはoutput数1の同じmodelとし、backend固有editor stateを持たない。Waylandのoutput別Dioxus instanceだけが`session_id`と単調な`revision`を持つ完全な`StageProjection`をtransport replicaとしてatomicに受け取り、replicaをauthorityにしない。compactな`ContextBar`、`EditorWorkspace`、`ObjectNavigator`、`Inspector`、`Accordion`、panel幅内`ActionBar`を同じDioxus treeで構成し、`Button`、`IconButton`、`TextField`、`NumberField`、`Toggle`、`ToggleGroup`、`SegmentedControl`、inline `ListPicker`、`AccordionSection`、`NavigatorTree`、`NavigatorItem`を共通controlとする。navigatorとInspectorは独立scrollし、treeとaccordionの開閉はsession内だけで保持する。browser-native select/dialog/popoverとglobal shortcutは使わず、Tab、Enter、Space、矢印key、Home、End、Escapeを各controlのcontractで扱う。text compositionはfocused controlのfield bufferとともにsessionへ保持し、候補確定中のEnterではfieldをcommitしない。fieldのcommitはreplica propsではなく順序づけられた入力でauthorityの最新bufferを検証・適用・終了する1 reducer transactionとする。displayからの右clickも共通Dioxus surface actionでeditorへ入る。canvasの右dragとwidgetの左dragではstart、全move、release、cancelをEditorSessionへreduceし、Dioxus memo/rebuildとWayland frameでpaintを合流する。Inspectorの数値fieldでもgeometryを編集し、canvas/widgetの位置とsizeは4px gridへ揃える。ただしcanvasの右端または下端が非gridのassigned-output外周へ正確に一致するdimensionは許可する。数値fieldは文字列bufferを保持し、Enterまたはblur時に有効値だけをdraftへ反映する。canvas名は各input eventでdraftへ反映し、空または重複をinline invalid stateとしてSave & Closeだけを無効にする。新規canvasはactive outputのouter boundsを`0,0,width,height`で正確にfillする。新規widgetは既定sizeでcanvas中央へ即時作成する。zero canvas時もoutputとskinを選択でき、CREATE FIRST CANVASはcurrent GAME SCREENだけに表示されるcanvasを作る。別outputのcanvasまたはwidgetをnavigatorで選択した場合は、そのconnected outputを同じreducer transactionでactiveにし、明示されたeditor開始時のGAME SCREENは上書きしない。output再割当とFIT TO OUTPUTは明示的なundo対象とし、navigationはdraftもundoも変更しない。canvas名、全6 GAME SCREENのvisibility、appearance、output、削除とwidget固有設定はInspectorに集約し、runtime refresh cadenceはeditor外とする。
...
通常のWayland visible surfaceはAUTO時にcompositor cadenceで描画し、整数指定時はそのHzを最大rasterization rateとする。公開eventとDioxus state更新はeventごとに維持し、各Wayland frameでDioxusを必ずpollする。変更がないpollはdamageやpaintを発生させず、複数のmemo/rebuild、skin更新とdrag変更は次の許可paintへ合流する。motionはmonotonic elapsed timeをsampleしてframeを飛ばし、遅く再生しない。初期configure、size/scale reconfigure、非表示時の透明clearは上限をbypassする。bounded native summaryは設定、paint数、経過時間、実効rateとbypass理由別件数を記録する。

editor preview中はcurrent GAME SCREENで表示中の全empty widgetに、inner apertureの`x,y · width×height`をcompactに重ね、drag/resizeへ追随する。座標原点はWayland output previewまたはOBS `/overlay` viewport左上であり、OBS sceneのPosition/Scale/Crop/Bounding Boxはscorepeekから観測できない。通常表示ではgeometryを出さない。
