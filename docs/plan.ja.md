...
### Live Overlay canvas（ADR 0122 / 0125 / 0127 / 0128 / 0129 / 0130 / 0135 / 0137 / 0138 / 0140 / 0143 / 0144 / 0145 / 0146）
...
editorはbackend全canvasを1 draftとして扱い、active output、current GAME SCREEN、canvas/widget選択、gesture、panel stateと一段のdocument UNDOを共有reducerで管理する。Wayland/OBS adapterはoutput catalog、surface/transportとinputだけを担当し、editor DOM/CSSも共有する。compactな`ContextBar`、`EditorWorkspace`、`ObjectNavigator`、`Inspector`、`Accordion`、panel幅内`ActionBar`を同じDioxus treeで構成し、`Button`、`IconButton`、`TextField`、`NumberField`、`Toggle`、`ToggleGroup`、`SegmentedControl`、inline `ListPicker`、`AccordionSection`、`NavigatorTree`、`NavigatorItem`、`StatusBadge`を共通controlとする。navigatorとInspectorは独立scrollし、treeとaccordionの開閉はsession内だけで保持する。browser-native select/dialog/popoverとglobal shortcutは使わず、Tab、Enter、Space、矢印key、Home、End、Escapeを各controlのcontractで扱う。canvasの右dragとwidgetの左drag、およびInspectorの数値fieldでgeometryを編集し、canvas/widgetの位置とsizeは4px gridへ揃える。ただしcanvasの右端または下端が非gridのassigned-output外周へ正確に一致するdimensionは許可する。数値fieldは文字列bufferを保持し、Enterまたはblur時に有効値だけをdraftへ反映する。canvas名は各input eventでdraftへ反映し、空または重複をinline invalid stateとしてSave & Closeだけを無効にする。新規canvasはactive outputのouter boundsを`0,0,width,height`で正確にfillする。新規widgetは既定sizeでcanvas中央へ即時作成する。zero canvas時もoutputとskinを選択でき、CREATE FIRST CANVASはcurrent GAME SCREENだけに表示されるcanvasを作る。output再割当とFIT TO OUTPUTは明示的なundo対象とし、navigationはdraftもundoも変更しない。canvas名、全6 GAME SCREENのvisibility、appearance、output、削除とwidget固有設定はInspectorに集約し、runtime refresh cadenceはeditor外とする。
