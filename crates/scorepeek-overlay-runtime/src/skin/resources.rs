...
impl StoreRoot {
    ...
    pub fn install(&self, source: &Path, force: bool) -> Result<InstallOutcome, String> {
        ...
        let result = (|| -> Result<InstallOutcome, String> {
            ...
            if !force
                && prior.as_ref().is_some_and(|(old, current)| {
                    *current && old.release == package.manifest.release
                })
            {
