...
pub fn operation_main(prefix: &[&str]) -> ExitCode {
    let mut arguments = vec![std::ffi::OsString::from("scorepeek-corpus")];
    arguments.extend(prefix.iter().map(std::ffi::OsString::from));
