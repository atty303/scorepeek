...
/// Writes a finite diagnostic snapshot and returns its semantic exit status.
///
/// # Errors
///
/// Returns an error when the selected run cannot be resolved or its stream is invalid.
pub fn inspect_latest(
    store: &Path,
    run_id: Option<&str>,
    output: &mut impl io::Write,
) -> Result<i32, String> {
    inspect_latest_with_format(store, run_id, InspectionFormat::Ndjson, output)
}
