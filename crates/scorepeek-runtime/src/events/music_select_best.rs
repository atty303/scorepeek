...
use super::server::{MusicSelectionState, SongPresentation};
#[cfg(test)]
...
pub struct MusicSelectResolverState {
    ...
    pub current_difficulty: Option<super::server::CurrentSelectionDifficulty>,
    pub difficulty_target: Option<super::server::SelectionDifficultyTarget>,
        ...
        && let Some(current) = state.current_difficulty
    {
        let _ = write!(
            ...
            match state.difficulty_target {
                Some(super::server::SelectionDifficultyTarget::Pending) => "pending",
                Some(super::server::SelectionDifficultyTarget::Incumbent) => "incumbent",
                Some(super::server::SelectionDifficultyTarget::Successor) => "successor",
                None => "-",
            }
        );
    }
