//! Default-remote `PipeWire` source discovery, admission, and lifecycle diagnostics.
...
pub(crate) const ITERATION_SLICE: Duration = Duration::from_millis(25);
...
pub struct UncalibratedPipewireSourceLease {
    pub(super) runtime: Option<DefaultRemoteRuntime>,
    pub(super) node_id: u32,
    pub(super) registry_global_count: u32,
    pub(super) started: Instant,
    pub(super) next_diagnostic_sequence: u64,
...
impl CaptureError {
    ...
    pub(crate) const fn without_source(error_type: CaptureErrorType) -> Self {
        Self {
            error_type,
            source: None,
        }
...
pub(super) struct TerminalError {
    ...
    pub(super) operation: CaptureDiagnosticOperation,
}
...
pub(super) struct DefaultRemoteRuntime {
...
pub fn probe_gamescope_source(
    timeout: Duration,
    sink: &mut impl CaptureDiagnosticSink,
) -> Result<PipewireSourceProbe, CaptureError> {
    let lease = acquire_gamescope_source(timeout, sink)?;
    let probe = PipewireSourceProbe {
...
pub fn snapshot_pipewire_sources(
    ...
    Ok(match (snapshot.candidate_count, snapshot.first_candidate) {
        (0, None) => PipewireSourceSnapshot::Absent,
        (1, Some(node_id)) => PipewireSourceSnapshot::Unique { node_id },
        _ => PipewireSourceSnapshot::Ambiguous {
