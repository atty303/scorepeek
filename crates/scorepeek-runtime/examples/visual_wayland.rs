...
fn load_document(
...
) -> Result<scorepeek_overlay_wayland::config::OverlayConfig, Box<dyn std::error::Error>> {
    if integration_fixture {
        let mut document = scorepeek_overlay_wayland::config::visual_debug_config(
    ...
    let Some(source_config) = source_config else {
        return Ok(scorepeek_overlay_wayland::config::OverlayConfig::initial());
    };
    let bytes = std::fs::read(&source_config)?;
    let document: scorepeek_overlay_wayland::config::OverlayConfig = toml::from_slice(&bytes)?;
    if document.schema_version != scorepeek_overlay_wayland::config::SCHEMA_VERSION {
