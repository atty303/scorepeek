...
#[allow(
    ...
    reason = "paired clap flags preserve explicit enable and disable overrides"
)]
struct RunArgs {
...
struct DiagnosticInspectArgs {
    /// Inspect the active run, or otherwise the most recent retained run.
...
#[command(
    name = "scorepeek-dev",
    version,
    about = "Dedicated development operations for scorepeek",
    disable_help_subcommand = true
)]
struct DevCli {
    /// Override the registered text-recognition model bundle.
...
#[derive(Subcommand)]
enum DevCommand {
    /// Run a low-level recognition development operation.
    Recognition {
        #[command(subcommand)]
        command: DevRecognitionCommand,
    },
}

#[derive(Subcommand)]
enum DevRecognitionCommand {
    Inspect(DevFrameArgs),
    Crop(DevFrameOutputArgs),
    MusicSelectCrop(DevFrameOutputArgs),
    TitleSpike(DevTitleSpikeArgs),
    FieldResourceLoadGate(DevResourceGateArgs),
    RecordingSimulationProfileAuthor(DevRecordingProfileAuthorArgs),
    RecordingSimulation(DevRecordingSimulationArgs),
    RecordingRecognitionEvidence(DevRecordingRecognitionArgs),
    RecordingRecognitionSimulation(DevRecordingRecognitionArgs),
    IntegratedContextCrop(DevFrameOutputArgs),
    IntegratedContextObserve(DevIntegratedObserveArgs),
    TitleOfficialDynamicOnnxDecode(DevDynamicDecodeArgs),
    TitleOfficialOnnxDecode(DevOfficialDecodeArgs),
    ProvisionalTitleCandidates(DevCatalogOutputArgs),
    TitleModelContractParity(DevModelContractParityArgs),
    TitleModelExportRequirements(DevModelExportArgs),
    TitleDictionaryAudit(DevDictionaryAuditArgs),
    TitleOnnxParity(DevOnnxParityArgs),
}
...
#[derive(Args)]
struct DevRecordingProfileAuthorArgs {
    #[arg(long)]
    candidate: PathBuf,
    #[arg(long)]
    candidate_sha256: String,
    #[arg(long)]
    recording_manifest: PathBuf,
    #[arg(long)]
    coverage_label: PathBuf,
    #[arg(long)]
    extraction: PathBuf,
    #[arg(long)]
    output: PathBuf,
...
#[derive(Args)]
struct DevRecordingSimulationArgs {
    #[arg(long)]
    profile: PathBuf,
    #[arg(long)]
...
#[derive(Args)]
struct DevRecordingRecognitionArgs {
    #[command(flatten)]
    simulation: DevRecordingSimulationArgs,
    #[arg(long)]
    recognition_artifact: PathBuf,
}

#[derive(Args)]
struct DevIntegratedObserveArgs {
    #[arg(long)]
    crop_artifact: PathBuf,
    #[arg(long)]
    crop_artifact_sha256: String,
    #[arg(long)]
    output: PathBuf,
}

#[derive(Args)]
struct DevDynamicDecodeArgs {
    #[arg(long)]
    model_id: String,
    #[arg(long)]
    bundle: PathBuf,
...
#[derive(Args)]
struct DevModelContractParityArgs {
    #[arg(long)]
    model: PathBuf,
    #[arg(long)]
    model_sha256: String,
...
#[derive(Args)]
struct DevModelExportArgs {
    #[arg(long)]
    catalog_store: PathBuf,
    #[arg(long)]
    baseline_dictionary: PathBuf,
    #[arg(long)]
    output: PathBuf,
}

#[derive(Args)]
struct DevDictionaryAuditArgs {
    #[arg(long)]
    catalog_store: PathBuf,
    #[arg(long)]
    dictionary: PathBuf,
...
struct DevOnnxParityArgs {
    ...
    dictionary: PathBuf,
    #[arg(long)]
    minimum_log_probability: f32,
    #[arg(long)]
    minimum_runner_up_margin: f32,
}

#[must_use]
pub fn public_main() -> ExitCode {
    let args = env::args_os().collect::<Vec<_>>();
    if matches!(args.as_slice(), [_, entrypoint] if entrypoint == scorepeek_overlay::children::ENTRYPOINT)
    {
        return exit_for_result(scorepeek_overlay::diagnostics::run());
    }
    if args.len() == 1 {
        let mut command = PublicCli::command();
    ...
    let cli = match PublicCli::try_parse_from(args) {
        Ok(cli) => cli,
        Err(error) => {
    ...
    exit_for_result(dispatch_public(cli))
}

#[must_use]
pub fn dev_main() -> ExitCode {
    let cli = match DevCli::try_parse() {
        Ok(cli) => cli,
        Err(error) => {
            let code = if error.use_stderr() { 2 } else { 0 };
            let _ = error.print();
    ...
    let mut args = cli.command.into_legacy_args();
    if let Some(bundle) = cli.model_bundle {
        args.splice(
            0..0,
            [OsString::from("--model-bundle"), bundle.into_os_string()],
        );
    }
...
impl DevCommand {
    fn into_legacy_args(self) -> Vec<OsString> {
        match self {
            Self::Recognition { command } => command.into_legacy_args(),
        }
    }
}

impl DevRecognitionCommand {
    #[allow(clippy::too_many_lines)]
    fn into_legacy_args(self) -> Vec<OsString> {
        let mut args = vec![OsString::from("recognition")];
        match self {
            Self::Inspect(frame) => {
                args.push("inspect".into());
                push_frame_args(&mut args, frame);
            }
            Self::Crop(output) => {
                args.push("crop".into());
                push_frame_output_args(&mut args, output);
            }
            Self::MusicSelectCrop(output) => {
                args.push("music-select-crop".into());
                push_frame_output_args(&mut args, output);
            }
            Self::TitleSpike(value) => {
                args.push("title-spike".into());
                push_dev_arg(&mut args, "--catalog-store", value.catalog_store);
                push_dev_arg(&mut args, "--ocr-text", value.ocr_text);
                push_dev_arg(
                    &mut args,
                    "--ocr-confidence",
                    value.ocr_confidence.to_string(),
                );
            }
            Self::FieldResourceLoadGate(value) => {
                args.push("field-resource-load-gate".into());
                push_dev_arg(&mut args, "--catalog-store", value.catalog_store);
                push_dev_arg(&mut args, "--catalog-sha256", value.catalog_sha256);
            }
            Self::RecordingSimulationProfileAuthor(value) => {
                args.push("recording-simulation-profile-author".into());
                push_dev_arg(&mut args, "--candidate", value.candidate);
                push_dev_arg(&mut args, "--candidate-sha256", value.candidate_sha256);
                push_dev_arg(&mut args, "--recording-manifest", value.recording_manifest);
                push_dev_arg(&mut args, "--coverage-label", value.coverage_label);
                push_dev_arg(&mut args, "--extraction", value.extraction);
                push_dev_arg(&mut args, "--output", value.output);
            }
            Self::RecordingSimulation(value) => {
                args.push("recording-simulation".into());
                push_recording_simulation_args(&mut args, value);
            }
            Self::RecordingRecognitionEvidence(value) => {
                args.push("recording-recognition-evidence".into());
                push_recording_recognition_args(&mut args, value);
            }
            Self::RecordingRecognitionSimulation(value) => {
                args.push("recording-recognition-simulation".into());
                push_recording_recognition_args(&mut args, value);
            }
            Self::IntegratedContextCrop(output) => {
                args.push("integrated-context-crop".into());
                push_frame_output_args(&mut args, output);
            }
            Self::IntegratedContextObserve(value) => {
                args.push("integrated-context-observe".into());
                push_dev_arg(&mut args, "--crop-artifact", value.crop_artifact);
                push_dev_arg(
                    &mut args,
                    "--crop-artifact-sha256",
                    value.crop_artifact_sha256,
                );
                push_dev_arg(&mut args, "--output", value.output);
            }
            Self::TitleOfficialDynamicOnnxDecode(value) => {
                args.push("title-official-dynamic-onnx-decode".into());
                push_dev_arg(&mut args, "--model-id", value.model_id);
                push_dev_arg(&mut args, "--bundle", value.bundle);
                push_dev_arg(&mut args, "--request", value.request);
            }
            Self::TitleOfficialOnnxDecode(value) => {
                args.push("title-official-onnx-decode".into());
                push_dev_arg(&mut args, "--model", value.model);
                push_dev_arg(&mut args, "--dictionary", value.dictionary);
                push_dev_arg(&mut args, "--request", value.request);
            }
            Self::ProvisionalTitleCandidates(value) => {
                args.push("provisional-title-candidates".into());
                push_dev_arg(&mut args, "--catalog-store", value.catalog_store);
                push_dev_arg(&mut args, "--output", value.output);
            }
            Self::TitleModelContractParity(value) => {
                args.push("title-model-contract-parity".into());
                push_dev_arg(&mut args, "--model", value.model);
                push_dev_arg(&mut args, "--model-sha256", value.model_sha256);
                push_dev_arg(&mut args, "--reference", value.reference);
                push_dev_arg(&mut args, "--reference-sha256", value.reference_sha256);
                push_dev_arg(&mut args, "--dictionary", value.dictionary);
            }
            Self::TitleModelExportRequirements(value) => {
                args.push("title-model-export-requirements".into());
                push_dev_arg(&mut args, "--catalog-store", value.catalog_store);
                push_dev_arg(
                    &mut args,
                    "--baseline-dictionary",
                    value.baseline_dictionary,
                );
                push_dev_arg(&mut args, "--output", value.output);
            }
            Self::TitleDictionaryAudit(value) => {
                args.push("title-dictionary-audit".into());
                push_dev_arg(&mut args, "--catalog-store", value.catalog_store);
                push_dev_arg(&mut args, "--dictionary", value.dictionary);
            }
            Self::TitleOnnxParity(value) => {
                args.push("title-onnx-parity".into());
                push_dev_arg(&mut args, "--model", value.model);
                push_dev_arg(&mut args, "--reference", value.reference);
                push_dev_arg(&mut args, "--reference-sha256", value.reference_sha256);
                push_dev_arg(&mut args, "--crop-artifact", value.crop_artifact);
                ...
                push_dev_arg(
                    ...
                    value.minimum_log_probability.to_string(),
                );
                push_dev_arg(
                    &mut args,
                    "--minimum-runner-up-margin",
                    value.minimum_runner_up_margin.to_string(),
                );
            }
        }
        args
...
fn push_dev_arg(args: &mut Vec<OsString>, flag: &'static str, value: impl Into<OsString>) {
    args.push(flag.into());
    args.push(value.into());
}

fn push_frame_args(args: &mut Vec<OsString>, frame: DevFrameArgs) {
    push_dev_arg(args, "--extraction", frame.extraction);
    push_dev_arg(args, "--extraction-sha256", frame.extraction_sha256);
    push_dev_arg(args, "--frame-id", frame.frame_id);
}

fn push_frame_output_args(args: &mut Vec<OsString>, output: DevFrameOutputArgs) {
    push_frame_args(args, output.frame);
    push_dev_arg(args, "--output", output.output);
}

fn push_recording_simulation_args(args: &mut Vec<OsString>, value: DevRecordingSimulationArgs) {
    push_dev_arg(args, "--profile", value.profile);
    push_dev_arg(args, "--profile-sha256", value.profile_sha256);
    push_dev_arg(args, "--extraction", value.extraction);
    push_dev_arg(args, "--diagnostic-root", value.diagnostic_root);
    push_dev_arg(args, "--catalog-store", value.catalog_store);
    push_dev_arg(args, "--run-id", value.run_id);
    push_dev_arg(args, "--build-sha256", value.build_sha256);
    push_dev_arg(args, "--recording", value.recording);
}

fn push_recording_recognition_args(args: &mut Vec<OsString>, value: DevRecordingRecognitionArgs) {
    push_recording_simulation_args(args, value.simulation);
    push_dev_arg(args, "--recognition-artifact", value.recognition_artifact);
}

fn exit_for_result(result: Result<(), String>) -> ExitCode {
...
fn run_diagnostic_command(command: DiagnosticCommand) -> Result<(), String> {
    match command {
        DiagnosticCommand::Observe { replay } => diagnostic_stream::observe(replay),
        DiagnosticCommand::Inspect(args) => {
            ...
            let status = diagnostic_stream::inspect_latest_with_format(
                &store,
...
fn run_skin_command(command: SkinCommand) -> Result<(), String> {
    let store = scorepeek_overlay::skin::StoreRoot::discover();
    match command {
        SkinCommand::Install { package } => {
            let outcome = store.install(&package)?;
            match outcome {
                scorepeek_overlay::skin::InstallOutcome::Installed => println!("installed"),
                scorepeek_overlay::skin::InstallOutcome::Replaced { previous_release } => {
                    println!("replaced {previous_release}");
                }
                scorepeek_overlay::skin::InstallOutcome::Unchanged => println!("unchanged"),
            }
        }
    ...
    match args {
        [skin, install, path] if skin == "skin" && install == "install" => {
            Some(store.install(Path::new(path)).map(|outcome| {
                use scorepeek_overlay::skin::InstallOutcome;
                match outcome {
...
fn run_routine_live_session(
    ...
    let overlay_config_path = overlays
        .config_path
        .unwrap_or_else(scorepeek_overlay::config::default_path);
    let requested_backends = [
        overlays
            .wayland
            .then_some(scorepeek_overlay::runtime::Backend::Wayland),
        overlays
            .obs
            .then_some(scorepeek_overlay::runtime::Backend::Obs),
    ];
    settle_output_startup_result(&mut output, monitor, Ok(()))?;
    let loaded_overlay_config_result = if overlays.wayland || overlays.obs {
        Some(scorepeek_overlay::config::load_or_create(
            &overlay_config_path,
        ))
        .transpose()
    } else {
    ...
    let overlay_controller = loaded_overlay_config
        .as_ref()
        .map(|(config, _)| {
            scorepeek_overlay::control::Controller::start(&overlay_config_path, config.clone())
        })
        .transpose();
...
mod tests {
    ...
    use super::{
        ...
        ConfigFile, DevCli, LIVE_SESSION_FLAGS, PrivatePublicationPoint, PublicCli, RunArgs,
    ...
    fn public_cli_exposes_only_the_seven_application_commands() {
        ...
        assert!(PublicCli::try_parse_from(["scorepeek", "recognition"]).is_err());
        assert!(PublicCli::try_parse_from(["scorepeek", "capture"]).is_err());
    }
...
"#,
    ...
    #[test]
    fn development_cli_accepts_options_independent_of_order() {
        ...
        .unwrap();
        let args = cli.command.into_legacy_args();
        assert_eq!(args[0], "recognition");
        assert_eq!(args[1], "inspect");
        assert_eq!(args[2], "--extraction");
    }
    ...
    fn run_diagnostics_start_before_config_loading_and_keep_the_failure_stage() {
        ...
        let mut diagnostics =
            crate::diagnostic_stream::RunDiagnostics::start(&diagnostic_store, run_id);
        let sink = diagnostics.sink();
        let monitor = crate::live_control::SignalStopMonitor::start().unwrap();
    ...
    fn public_run_interrupt_prioritizes_cancel_over_startup_success_and_failure() {
        ...
        for mode in ["success", "failure"] {
            ...
            let status = Command::new(std::env::current_exe().unwrap())
                .args([
                    "--exact",
                    "application::tests::public_run_interrupt_prioritizes_cancel_over_startup_success_and_failure",
                    "--nocapture",
                ])
                .env(CHILD_MODE, mode)
    ...
    fn output_owned_diagnostics_prioritize_interrupt_over_startup_failure() {
        ...
        if let Some(root) = std::env::var_os(CHILD_ROOT) {
            let diagnostics = crate::diagnostic_stream::RunDiagnostics::start(
                Path::new(&root),
                "run-output-interrupt-test",
            );
            let mut output = crate::routine_output::RoutineOutput::start_headless_with_diagnostics(
        ...
        let status = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "application::tests::output_owned_diagnostics_prioritize_interrupt_over_startup_failure",
                "--nocapture",
            ])
