...
use super::activation::{ActiveCatalog, CatalogOrigin, CatalogUpdate};
use scorepeek_core::catalog::policy::{
    Completeness, LineageId, RevisionStrategy, SourceId, SourcePolicy,
};
use scorepeek_core::catalog::{
    Catalog, CatalogSong, Chart, ChartAssertion, ChartKey, Difficulty, DisplayVariant,
    DisplayVariantKind, DqnBinding, EvidenceId, ExactTitleArtist, InfinitasStatus, PlayType,
...
impl CatalogUpdate {
    ...
    /// Installs already verified snapshot bytes while holding this update's writer lock.
    ///
    /// # Errors
    /// Returns an error if the digest, capacity, or durable activation contract fails.
    pub fn install_verified_snapshot(
        self,
        source: &Path,
        expected_digest: &str,
        ...
        validate_digest(expected_digest)?;
        self.install_snapshot(source, expected_digest, None)
    }
    ...
    fn activate_digest(
        ...
        checkpoint(PublishPoint::FileSynced)?;
        ...
        checkpoint(PublishPoint::Renamed)?;
        File::open(self.store.manifest_dir())?.sync_all()?;
        checkpoint(PublishPoint::ParentSynced)?;
...
mod tests {
    ...
    use scorepeek_core::catalog::test_support::{
        SyntheticTachiRecord, catalog_from_tachi, federate_tachi,
    };
    use scorepeek_core::catalog::{Catalog, Chart, ChartKey, Difficulty, PlayType};
    ...
    fn every_publish_failure_exposes_old_or_complete_new_snapshot() {
        let points = [
            ...
            PublishPoint::FileSynced,
            PublishPoint::Renamed,
            PublishPoint::ParentSynced,
        ];
        for point in points {
            let root = TempDir::new().unwrap();
            let store = CatalogStore::new(root.path());
            ...
            if matches!(point, PublishPoint::Renamed | PublishPoint::ParentSynced) {
    ...
    fn publish_rejects_a_changed_base_manifest() {
        ...
        let error = update
            .publish_with(&synthetic_catalog("NEW"), |point| {
                if point == PublishPoint::FileSynced {
