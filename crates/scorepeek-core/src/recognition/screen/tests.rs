...
fn assert_integrated_artifact_is_strict(
    ...
    assert!(read_integrated_context_crop_artifact(directory, &summary.manifest_sha256).is_err());
}
...
fn integrated_context_crops_keep_the_base_layout_stable() {
    ...
    let result_summary =
        export_integrated_context_crops(&test_frame(result_pixels), "result-001", &result_output)
            .unwrap();
...
fn canonical_frame_requires_bound_normalizer_evidence() {
    ...
    assert!(
        CanonicalFrame::read_extraction(directory.path(), "result-001", &"9".repeat(64)).is_err(),
    ...
    assert!(
        CanonicalFrame::read_extraction(directory.path(), "result-001", &invalid_manifest_sha256,)
            .is_err(),
    ...
    assert!(
        CanonicalFrame::read_extraction(directory.path(), "result-001", &manifest_sha256).is_err(),
