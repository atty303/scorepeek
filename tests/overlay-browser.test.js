...
const baseURL = Deno.env.get("SCOREPEEK_BROWSER_TEST_URL");

browserTest(
  "asset version mismatch reloads the editor automatically",
  async ({ page }) => {
...
browserTest(
  "repeated asset version mismatch stops after one automatic reload",
  async ({ page }) => {
...
browserTest(
  ...
  async ({ page }) => {
    ...
    page.on("framenavigated", (frame) => {
      if (frame.url().includes("/canvas/")) {
        replicaLifecycle.push({ phase: "loaded", url: frame.url() });
      }
    });
    page.on("framedetached", (frame) => {
      if (frame.url().includes("/canvas/")) {
        replicaLifecycle.push({ phase: "destroyed", url: frame.url() });
      }
    });
    page.on("request", (request) => {
      if (
        request.url().includes("/skin/") && request.url().endsWith("/skin.wasm")
      ) {
        skinWasmRequests.push(request.url());
      }
    });
    await page.addInitScript(() => {
      globalThis.__scorepeekTest = {
        presentations: [],
        ...
        if (typeof message === "string") {
          try {
            ...
            if (envelope.request?.command === "update_backend_draft") {
              globalThis.__scorepeekTest.draftUpdates.push(envelope);
            }
      ...
      addEventListener("message", (event) => {
        ...
        try {
          ...
          if (message.type === "scorepeek-editor-presentation") {
            globalThis.__scorepeekTest.presentations.push(message);
            if (
              message.session_id !== globalThis.__scorepeekTest.lastSession ||
              message.revision > globalThis.__scorepeekTest.lastRevision
            ) {
              globalThis.__scorepeekTest.lastSession = message.session_id;
              globalThis.__scorepeekTest.lastRevision = message.revision;
              globalThis.__scorepeekTest.acceptedPresentations.push(message);
            }
          }
        } catch { /* Ignore non-JSON messages. */ }
      }, { capture: true });
    ...
    await page.routeWebSocket("**/ws/canvas-*", async (client) => {
      ...
      server.onMessage((message) => {
        ...
        try {
          ...
          if (envelope.type === "presentation") {
            canvasPresentations.push(envelope.specification);
          }
    ...
    const addCanvas = page.getByRole("button", {
      name: "+ Add canvas",
      exact: true,
    });
    ...
    await page.locator("#widget-picker-trigger").click();
    await page.locator(".list-picker-option[data-index='0']").click();
    const secondWidget = page.locator(
      ".editor-canvas[data-canvas='canvas-2'] .editor-widget-hit",
    );
    for (const field of ["x", "y"]) {
      const input = page.locator(`[id='canvas-2:status-1:${field}']`);
      await input.fill("0");
      await input.press("Enter");
      await expect.poll(async () => (await secondWidget.boundingBox())[field])
        .toBe(0);
    }
    ...
    for (
      ...
      await input.blur();
      await expect.poll(async () => (await secondCanvas.boundingBox())[field])
        .toBe(Number(value));
    }
    ...
    const candidateHandle = page.locator(
      ".editor-canvas[data-canvas='canvas-1'] .editor-widget-hit",
    ).last();
    await candidateHandle.click();
    const draggedWidgetId = await page
      .locator(
        ".editor-canvas[data-canvas='canvas-1'] .editor-widget-hit.selected",
      )
    ...
    const connectionsBeforeRestart =
      canvasConnections.filter(({ url }) => url.includes("/ws/canvas-1"))
        .length;
    const statesBeforeRestart = canvasStates.length;
    const presentationsBeforeRestart = canvasPresentations.length;
    const replicaLoadsBeforeRestart =
      replicaLifecycle.filter(({ phase, url }) =>
        phase === "loaded" && url.includes("/canvas/canvas-1")
      ).length;
    const replicaURLBeforeRestart = await page.locator(
      "#scorepeek-replica-canvas-1",
    )
      .getAttribute("src");
    rejectedCanvasConnections = 2;
    const activeCanvasConnection = canvasConnections.findLast((
      { server, url },
    ) => server && url.includes("/ws/canvas-1"));
    expect(activeCanvasConnection).toBeDefined();
    await activeCanvasConnection.client.close({
      code: 1012,
      reason: "backend restarting",
    });
    await expect.poll(
      () =>
        canvasConnections.filter(({ url }) => url.includes("/ws/canvas-1"))
    ...
    const updateCountBeforeDrag = await page.evaluate(() =>
      globalThis.__scorepeekTest.draftUpdates.length
    );
    const initialReplicaFrame = page.frames().find((candidate) =>
      candidate.url().includes("/canvas/canvas-1")
    );
    expect(initialReplicaFrame).toBeDefined();
    const presentationCountBeforeDrag = await initialReplicaFrame.evaluate(
      () => globalThis.__scorepeekTest.acceptedPresentations.length,
    );
    ...
    await expect.poll(() => handle.boundingBox()).toEqual(expected);
    await expect.poll(() => slot.boundingBox()).toEqual(expected);
    expect(await unrelatedSameCanvas.boundingBox()).toEqual(
      unrelatedBefore.sameCanvas,
    );
    ...
    const dragUpdates = await page.evaluate(
      ...
      updateCountBeforeDrag,
    );
    expect(dragUpdates).toHaveLength(1);
    const persistedCanvas = dragUpdates[0].request.canvases.find((candidate) =>
      candidate.id === "canvas-1"
    );
    const persistedWidget = persistedCanvas.widgets.find(({ id }) =>
      id === draggedWidgetId
    );
    ...
    const replicaFrame = page.frames().find((candidate) =>
      candidate.url().includes("/canvas/canvas-1")
    );
    expect(replicaFrame).toBeDefined();
    const replicaRevisions = await replicaFrame.evaluate(
      () =>
        globalThis.__scorepeekTest.acceptedPresentations.map((
          { session_id, revision, specification },
        ) => ({
          session_id,
          revision,
          geometry: specification.widgets.map((
            { x, y, width, height },
          ) => [x, y, width, height]),
        })),
    );
    expect(replicaRevisions.length).toBeGreaterThan(0);
    expect(replicaRevisions.every((current, index, values) =>
      index === 0 ||
      current.session_id !== values[index - 1].session_id ||
      current.revision > values[index - 1].revision
    )).toBe(true);
    const dragReplicaRevisions = replicaRevisions.slice(
      presentationCountBeforeDrag,
    );
    ...
    const draggedWidgetIndex = persistedCanvas.widgets.findIndex(({ id }) =>
      id === draggedWidgetId
    );
    expect(draggedWidgetIndex).toBeGreaterThanOrEqual(0);
    expect(
      new Set(
        dragReplicaRevisions.map(({ geometry }) =>
          JSON.stringify(geometry[draggedWidgetIndex])
        ),
      ).size,
    )
      .toBeGreaterThanOrEqual(8);
    expect(replicaRevisions.at(-1).geometry[draggedWidgetIndex]).toEqual(
      persistedGeometry,
    );
    ...
    await page.mouse.move(
      firstDragEnd.x + firstDragEnd.width / 2,
      firstDragEnd.y + firstDragEnd.height / 2,
    );
    await page.mouse.down();
    ...
    await expect.poll(() => handle.boundingBox()).toEqual(
      delayedFirstExpected,
    );
    const secondDragStart = await handle.boundingBox();
    await page.mouse.move(
      secondDragStart.x + secondDragStart.width / 2,
      secondDragStart.y + secondDragStart.height / 2,
    );
    await page.mouse.down();
    for (let step = 1; step <= 4; step += 1) {
      await page.mouse.move(
    ...
    await expect.poll(() => slot.boundingBox()).toEqual(
      consecutiveExpected,
    );
    await expect.poll(() => {
      ...
      const widget = presentation?.widgets.find(({ id }) =>
        id === draggedWidgetId
      );
    ...
    const canvasScrollPoint = await page.locator(".canvas-select").first()
      .evaluate((node) => {
        const row = node.getBoundingClientRect();
        const viewport = node.closest(".navigator-scroll")
          .getBoundingClientRect();
        return {
          x: row.left + row.width / 2,
          y: Math.max(
            viewport.top + 2,
            Math.min(row.top + row.height / 2, viewport.bottom - 2),
          ),
        };
      });
    await page.mouse.move(canvasScrollPoint.x, canvasScrollPoint.y);
    await page.mouse.wheel(0, 600);
    await expect.poll(() => navigator.evaluate((node) => node.scrollTop))
      .toBeGreaterThan(0);
    await navigator.evaluate((node) => {
      node.scrollTop = 0;
    });
    ...
    await expect.poll(() => inspector.evaluate((node) => node.scrollTop))
      .toBeGreaterThan(0);

    const visibility = page
      .locator(
        ".editor-accordion-heading[data-section='canvas:canvas-1:visibility']",
      )
      .locator("..");
    ...
    await expect.poll(() =>
      replicaLifecycle.slice(lifecycleBeforeVisibility)
        .some((event) => event.phase === "destroyed")
    ).toBe(true);
    ...
    await expect.poll(() => skinWasmRequests.length).toBeGreaterThan(
      wasmBeforeVisibility,
    );

    const appearance = page
      .locator(
        ".editor-accordion-heading[data-section='canvas:canvas-1:appearance']",
      )
      .locator("..");
    await appearance
      .locator(
    ...
    const initialFrameURL = await page.locator("#scorepeek-replica-canvas-1")
      .getAttribute("src");
    ...
    await expect(slot).toBeVisible();
    await expect.poll(() =>
      replicaLifecycle.slice(lifecycleBeforeSkin)
        .filter((event) => event.phase === "loaded").length
    ).toBeGreaterThan(0);
    await expect.poll(() => skinWasmRequests.length).toBeGreaterThan(
      wasmBeforeSkin,
    );
    ...
    await page.getByRole("button", { name: "Canvas 1", exact: true }).click();
    for (
      const [field, value] of [["x", "2001"], ["y", "-101"], ["width", "701"]]
    ) {
    ...
    await expect(
      page.getByRole("button", { name: "Save & Close", exact: true }),
    ).toBeEnabled();
    await page.getByRole("button", { name: "Save & Close", exact: true })
      .click();
    await expect(page.locator(".editor-panel")).toHaveCount(0);
    ...
    await page.reload();
    await expect.poll(() => stageConnections.length).toBe(
      stageConnectionsBeforeReload + 2,
    );
    stageConnections.at(-1).release();
    await page.locator("#stage").click({
      button: "right",
    ...
    await page.locator(
      ".canvas-select[data-canvas-id='canvas-1'] .widget-row .navigator-item-select",
    ).first().click();
    const offscreenWidgetX = page.locator(
      ".object-inspector input[id^='canvas-1:'][id$=':x']",
    );
