//! History, graph and aperture renderers for the baseline skin.
...
fn plot_point(
    ...
    let x = normalized_time_position(time, start, end) * 1000.0;
    let y = (1.0 - ratio.clamp(0.0, 1.0)) * view_height;
    Some((x, y))
}

fn normalized_time_position(time: i64, start: i64, end: i64) -> f64 {
    let span = i128::from(end.saturating_sub(start).max(1));
    let elapsed = i128::from(time.saturating_sub(start)).clamp(0, span);
    let millionths = elapsed.saturating_mul(1_000_000) / span;
    f64::from(u32::try_from(millionths).unwrap_or(0)) / 1_000_000.0
}
...
fn graph_axes_and_ticks(key: &str, start: i64, end: i64, state: &Value) -> (Node, Node, Vec<Node>) {
    let ranks = [
        ("AAA", 8_u8),
        ("AA", 7),
        ("A", 6),
    ...
    let ticks = state
                ...
                element(
                    ...
                    &[
                        ...
                        (
                            ...
                            format!(
                                "left:{:.2}%",
                                normalized_time_position(time, start, end) * 100.0
                            ),
                        ),
                    ],
                    vec![Node::text(
        ...
        .collect();
    (rank_axis, miss_axis, ticks)
}

fn graph_curves(key: &str, state: &Value, start: i64, end: i64, view_height: f64) -> Vec<Node> {
...
pub(super) fn history_graph(key: &str, widget: &Widget, state: &Value) -> Node {
    ...
    let curves = graph_curves(key, state, start, end, view_height);
    let (rank_axis, miss_axis, ticks) = graph_axes_and_ticks(key, start, end, state);
    panel(
        key,
        "history-graph-panel",
        widget,
...
pub(super) fn empty(key: &str, widget: &Widget) -> Node {
        ...
        element(
            ...
            &[
                ...
                (
                    "style",
                    format!(
                        "background:rgba(4,13,22,{:.2})",
                        f64::from(u32::try_from(opacity).unwrap_or(100)) / 100.0
                    ),
                ),
            ],
            vec![],
    ...
    element(
        ...
        "section",
        &[
            ("class", "base-panel empty-panel".into()),
