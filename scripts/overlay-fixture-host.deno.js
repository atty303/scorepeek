...
const [root, address, sceneArgument, configArgument, backendArgument] = process
  .argv.slice(2);
const scenePath = sceneArgument === "-" ? null : sceneArgument;
const backend = backendArgument || "obs";
if (
  !root || !address || (scenePath && !path.isAbsolute(scenePath)) ||
  (configArgument && !path.isAbsolute(configArgument))
) {
  throw new Error(
    "usage: overlay-fixture-host.js ROOT 127.0.0.1:PORT [ABSOLUTE_SCENE.json|-] [ABSOLUTE_CONFIG.toml] [obs|wayland]",
  );
}
if (!["obs", "wayland"].includes(backend) || (scenePath && backend !== "obs")) {
  throw new Error("invalid fixture backend");
}
if (!/^127\.0\.0\.1:\d+$/.test(address)) {
  throw new Error("fixture listener must be loopback");
}
const dataHome = process.env.XDG_DATA_HOME;
if (
  !dataHome || !process.env.XDG_CONFIG_HOME || !process.env.XDG_STATE_HOME ||
  !process.env.XDG_CACHE_HOME || !process.env.XDG_RUNTIME_DIR ||
  !process.env.HOME
) {
  throw new Error(
    "fixture requires isolated HOME and every XDG base directory",
  );
}
const scene = scenePath ? JSON.parse(fs.readFileSync(scenePath, "utf8")) : null;
if (scene && scene.schema !== "scorepeek-skin-preview-scene-v1") {
...
const initialCanvases = backend === "wayland"
  ? ["HEADLESS-1", "HEADLESS-2"].map((output, index) => ({
    id: `wayland-${index + 1}`,
  ...
  : scene
  ? scene.skins.map((skin) => ({
    id: `preview-${skin.slug}`,
    name: skin.slug,
    backend: "obs",
    skin: skin.id,
  ...
  : [];
const saved = initialCanvases.map(({ backend: _backend, ...canvas }) => ({
  ...canvas,
...
function tomlDocument(canvases) {
  ...
  for (const canvas of canvases) {
    ...
    if (canvas.skin_properties && Object.keys(canvas.skin_properties).length) {
      ...
      for (const [key, value] of Object.entries(canvas.skin_properties)) {
        lines.push(`${tomlValue(key)} = ${tomlValue(value)}`);
      }
    }
    for (const widget of canvas.widgets || []) {
      lines.push("", "[[canvases.widgets]]");
      for (const key of ["id", "kind", "x", "y", "width", "height"]) {
        lines.push(`${key} = ${tomlValue(widget[key])}`);
      }
      for (const key of ["skin_properties", "settings"]) {
        if (widget[key] && Object.keys(widget[key]).length) {
          lines.push("", `[canvases.widgets.${key}]`);
          for (const [property, value] of Object.entries(widget[key])) {
            lines.push(`${tomlValue(property)} = ${tomlValue(value)}`);
          }
        }
      }
    }
...
function control(request) {
  if (request.backend !== backend) {
    return { ...response(true), ok: false, error: "unsupported backend" };
  }
  switch (request.command) {
    case "acquire_backend":
      if (lease && lease !== request.editor_id) {
        return {
          ...response(true),
          ok: false,
          error: "workspace editor is already active",
        };
      }
      if (!lease) {
        lease = request.editor_id;
        active = structuredClone(saved);
        generation += 1;
      }
    ...
    case "commit_backend":
      if (lease !== request.editor_id) {
        return { ...response(true), ok: false, error: "editor lease lost" };
      }
...
const controller = net.createServer((socket) => {
  ...
  socket.on("data", (bytes) => {
    ...
    let result;
    try {
      result = control(JSON.parse(input.slice(0, end)));
    } catch (error) {
      result = { ...response(true), ok: false, error: String(error) };
    }
...
controller.listen(controlSocket, () => {
    ...
    feedServer = net.createServer((socket) => {
      socket.write(
        JSON.stringify({
          ...
          invocation_id: "nested-wayland-fixture",
          next_sequence: 1,
          status: {
          ...
          result: {
            schema: "scorepeek-event-v5",
            invocation_id: "nested-wayland-fixture",
            sequence: 0,
            event_id: "nested-wayland-fixture:0",
            emitted_monotonic_ms: 0,
            emitted_unix_ms: 1000,
            capture: null,
  ...
  const config = {
    ...
    socket: backend === "wayland"
      ? eventSocket
      : path.join(root, "absent-events.sock"),
    invocation: "overlay-fixture",
    scores_db: null,
    unknown_grace_ms: 1000,
    ...(backend === "wayland" ? { edit_on_start: true } : { listen: address }),
  };
  child = spawn(path.resolve("target/debug/scorepeek"), [
    backend === "obs"
      ? "__scorepeek-overlay-web-host"
      : "__scorepeek-overlay-wayland",
  ], {
    stdio: [
      "pipe",
      fs.openSync(path.join(root, "role.stdout"), "w"),
      fs.openSync(path.join(root, "role.stderr"), "w"),
    ],
    env: process.env,
  });
  ...
  child.on("exit", (code, signal) => {
    if (!stopping) {
      process.stderr.write(`overlay role exited: ${code ?? signal}\n`);
      process.exitCode = 1;
    }
    controller.close();
    process.stdin.pause();
