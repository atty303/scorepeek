...
cleanup() {
  ...
  if (( status != 0 )); then
    sed -n '1,240p' "${test_root}/gamescope.log" >&2 || true
...
wait_for_session() {
  ...
  for _ in $(seq 1 200); do
    if find "${state_root}/scorepeek/diagnostics" -name diagnostics.ndjson -type f -exec grep -q 'capture_generation_identity' {} + 2>/dev/null; then
      return
    fi
    if ! kill -0 "${scorepeek_pid}" 2>/dev/null; then
      wait "${scorepeek_pid}"
  ...
  echo "Vulkan capture session was not admitted" >&2
  exit 1
}
